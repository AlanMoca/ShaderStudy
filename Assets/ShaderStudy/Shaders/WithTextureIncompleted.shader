Shader "Unlit/WithTextureIncompleted"
{
		...
		SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(VertexOutput o) : SV_Target
			{
				...
				//return sin(o.uv0.y * 16);					//Activar

				//return frac(_Time.y);			//Frac va de su valos más bajo al más alto por lo que entendí por eso da el efecto de tintillar! frac = value - floor(value);

				/*float waveSize = 0.01;
				float shape = o.uv0.y;*/
				//return (sin(o.uv0.y * 16)+1) * 0.5;					//Activar
				//return (sin(shape / waveSize) + 1) * 0.5;					//Activar

				//float shoreline = tex2D(_ShorelineTex, o.uv0).x;
				//float shapeDos = shoreline
				//return (sin(shapeDos / waveSize) + 1) * 0.5;					//Activar
				
				//float shoreline = tex2D(_ShorelineTex, o.uv0).x;
				//float waveSize = 0.04;
				//float shapeDos = o.uv0.y;
				////float shapeDos = shoreline
				//return (sin(shapeDos / waveSize + _Time.y * 4) + 1) * 0.5;					//Activar

				float shoreline = tex2D(_ShorelineTex, o.uv0).x;
				float waveSize = 0.04;
				float shapeDos = o.uv0.y;
				//float shapeDos = shoreline
				float waveAmp = (sin(shapeDos / waveSize + _Time.y * 4) + 1) * 0.5;					//Activar
				waveAmp *= o.uv0.y;
				//waveAmp *= shoreline;
				...
				////Algo como interpolación y gradiente
				//float3 colorA = float3(0.1, 0.8, 1);
				//float3 colorB = float3(1, 0.1, 0.8);
				//float t = uv.y;
				//float3 blend = lerp(colorA, colorB, t);		//Lerp es una interpolación basada en el 3er valor (supongo que define en qué momento se degradará más como step :V)
				//return float4(blend, 0);

				//En este ejemplo se retorna la celda de sombras del shader y yo decido (8) el número de sombras.
				//remember t=uv.y
				/*t = round(t * 8) / 8;			//option1
				t = floor(t * 8) / 8;			//option1
				return t;*/
				...
				lightFalloff = step(0.1, lightFalloff);							// CARTOON Es para definir qué tanto se difuminará la luz? (Reduce el número de sombras)
				...
				specularFalloff = step(0.1, specularFalloff);							// CARTOON Es para definir qué tanto se difuminará la luz?
				//specularFalloff = Posterize(8, specularFalloff);						// Subdivide la luz en su celda de sombras en 8 (Activar Y definir una función)
