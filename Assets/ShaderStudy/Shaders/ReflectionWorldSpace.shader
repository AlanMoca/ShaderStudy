Shader "Unlit/ReflectionWorldSpace"						// El problema es que si rotas la sphere el punto donde refleja like a diamond, rota con él y obvio se ve falso.
{
	Properties																			//Todas las cosas que definiran el material (inputs)
	{
		...
		_Gloss("Gloss", Range(0,1)) = 1						//Esto será con lo que lo modificarás en el inspector de unity! D: (ES UN RANGE PARA REMAPPING GLOSS)
		...
		SubShader
	{
		...
		Pass
		{
			...
			VertexOutput vert(VertexInput v)
			{
				VertexOutput o;
				o.uv0 = v.uv0;
				//o.normal = v.normal;									// La normal en esta línea se la pasa por la posición local del modelo
				o.normal = UnityObjectToWorldNormal(v.normal);			// Aqué la normal local la convertiremos a normal de mundo! Así dependerá de los ejes del mundo.
			...
			fixed4 frag(VertexOutput o) : SV_Target
			{
				...
				//return float4(normal, 1);						// Debug de normal de world
				...
				float specularExponent = exp2(_Gloss * 11);					//ESTO LIMITA EL GLOSS A 1 PARA QUE SEA MAS LINEAL 1 > 2048 (2^11 ES EL MAXIMO VALOR AL QUE LLEGABA EL GLOSS)

				//Modify Gloss
				specularFalloff = pow(specularFalloff, specularExponent);						//El vector resultante del producto punto entre el vector reflection y el vector luz (specularFalloff), elevado a la potencia que se le de el Gloss.
																					//Bien lo que esto resultará es qué tanto brillo (gloss), dará del punto de refleción! 
				specularFalloff *= _Gloss;											//Esto es para que se vea mejor el difuminado de la luz, en un punto se verá concentrado, en varios difuminado. (Fake Effect Energy conservation, the true one is for PBR)
